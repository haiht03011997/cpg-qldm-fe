...
import { fetchAccountOwner } from 'app/shared/reducers/accountOwner/view/accountOwner.reduces';
...
import './style.scss';
import { deleteAccountOwner } from 'app/shared/reducers/accountOwner/form/accountOwner.reduces';
import { getActionRolesByPath } from 'app/shared/util/authentication';

const { Search } = Input;

const AccountOwnerListComponent = () => {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const location = useLocation();
  const { filter } = location.state || {};
  const { data, isFetching, page, limit, searchText, total } = useAppSelector(state => state.accountOwnerView);
        ...
        <div className="action">
          {roles.update && (
            <span className="action-edit">
              <Tooltip title="Sửa">
                <Link state={{ filter: filters }} to={`/account-owner/${item.accountOwnerId}`}>
          ...
          {roles.delete && (
            <span className="action-delete">
              <Tooltip title="Xóa">
                <FontAwesomeIcon
                  onClick={() => {
                    handleWarningDeleteItem(item.accountOwnerId);
                  }}
                  icon="trash"
                />
              </Tooltip>
  ...
  const handleFetchData = payload => {
    dispatch(fetchAccountOwner(payload));
  };
  ...
  const handleCreateAccountOwner = () => {
    navigate('create', {
  ...
  const handleConfirmDelete = () => {
    if (idDeleteSelected) {
      dispatch(deleteAccountOwner(idDeleteSelected))
        .then(() => {
  ...
  return (
    <Container className="accountOwner-view">
      <Title level={3}>Danh sách thông tin chủ quản</Title>
      <div className="table">
      ...
      <ModalConfirmComponent
        ...
        title="Bạn có muốn xóa chủ quản"
      />
    </Container>
  );
};

export default AccountOwnerListComponent;
