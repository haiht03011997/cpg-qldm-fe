import { Button, Col, DatePicker, Form, Input, InputNumber, Modal, Row, notification } from 'antd';
...
const AppendixFormComponent = (props: IAppendixModal) => {
  ...
  const handleSubmit = values => {
    const contractAddendumCollateralId = props.id;
    const payload = {
      ...detail,
      ...values,
      collateralId: id,
  ...
  return (
    ...
    >
      <Form className="multiple-form-horizontal-collateral" size="large" form={form} onFinish={handleSubmit} layout="vertical">
        <Title level={3}>{`${props.id ? 'Cập nhật' : 'Thêm mới'} phụ lục hợp đồng`}</Title>
        <Row>
          <Col md={11}>
        ...
        <Row>
          <Col md={11}>
            <Form.Item label="Số tiền vay (đ)" name={'loanAmount'} initialValue={0}>
              <InputNumber formatter={formatVND} parser={parseFloatVND} className="w-100" placeholder="Nhập vào số tiền vay" />
            </Form.Item>
          </Col>
          <Col md={1}></Col>
          <Col md={12}>
...
export default AppendixFormComponent;
