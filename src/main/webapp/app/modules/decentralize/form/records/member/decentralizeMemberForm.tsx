...
const DecentralizeMemberCompanyForm = ({ userLogin, handlePayload }: IDecentralizeRecord) => {
  const { data: detail } = useAppSelector(state => state.decentralizeForm);
  const { data: treeMemberCompany } = useAppSelector(state => state.buildTreeMemberCompany);
  ...
  const handleDefaultCheckedKeyRecords = arrayRecords => {
    ...
    arrayRecords.forEach(element => {
      const item = findItemByAction(element, treeMemberCompany);
      if (item) {
        if (item.key) {
  ...
  const processKeys = (item, defaultKeys, actions) => {
    ...
    if (_.find(actions, act => act === ACTION.ALL)) {
      ...
      if (childKeys?.length > 0) {
        const keys = _.filter(childKeys, key =>
          _.some(actions, act => {
            const [, value] = key.split('/'); // Split each element by '/' and get the second part
            return actions.includes(value); // Check if the second part is present in array2
          }),
        );
  ...
  const handleFormPayload = () => {
    ...
    const arrayOfObjects = Object.entries(groupedActions).map(([companyId, actions]: any) => {
      const result: IDecentralize = {
        userLogin,
        type: 'companyMember', // Đối tượng
        companyId, // Id công ty chứng khoán
        permission: 'p2', // policy casbin
        actions,
      };
  ...
  return (
    <Tree checkable autoExpandParent={autoExpandParent} onCheck={onCheck} checkedKeys={checkedKeyRecords}>
      {renderTreeNodes(treeMemberCompany)}
    </Tree>
  );
};
export default DecentralizeMemberCompanyForm;
